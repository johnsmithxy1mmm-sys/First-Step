...
MUTANTS: list[tuple[str, str, str, str]] = [
    ...
    ("marketmaker.py", "        worst_price = max(yes_bid, no_bid, tick)",
     "        worst_price = max(yes_bid, tick)",
     "quote sized off the cheap leg (a 1.8c market bought 60x the budget)"),
    ("marketmaker.py",
     "        if abs(self._inventory_usd(market)) + size * worst_price \\",
     "        if abs(self._inventory_usd(market)) + size * yes_bid \\",
     "per-market cap measures the cheap leg and cannot refuse the real risk"),
    ("marketmaker.py",
