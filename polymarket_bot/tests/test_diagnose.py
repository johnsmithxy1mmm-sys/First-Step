"""Диагностика воронки: причины отсева для MM и лонгшот-сканера."""
...
def test_mm_reject_reasons(cfg):
    ...
    assert "объём" in s.reject_reason(mm_ok(volume_24h_usd=1000))
    assert "rewards" in s.reject_reason(mm_ok(rewards_min_size=0))
    assert "резолюции" in s.reject_reason(
        mm_ok(end_date=datetime.now(timezone.utc) + timedelta(days=3)))
    assert "волатил" in s.reject_reason(mm_ok(one_day_price_change=0.2))
    assert "спред" in s.reject_reason(mm_ok(best_bid=0.449, best_ask=0.450))
...
def test_scanner_reject_reasons(cfg):
    sc = Scanner(cfg)
    good = make_market(outcome_prices=[0.01, 0.99])   # дешёвый Yes есть
    assert sc.reject_reason(good) is None
    assert "объём" in sc.reject_reason(make_market(volume_24h_usd=100))
    assert "окна" in sc.reject_reason(make_market(end_date=None))
    assert "цене" in sc.reject_reason(make_market(outcome_prices=[0.30, 0.70]))
...
def test_funnel_counts(cfg):
    s = MarketScorer(cfg)
    markets = [
        mm_ok(id="a"), mm_ok(id="b"),                     # 2 проходят
        mm_ok(id="c", volume_24h_usd=1000),               # объём
        mm_ok(id="d", volume_24h_usd=1000),               # объём
        mm_ok(id="e", rewards_min_size=0),                # rewards
    ]
    reasons, passed = funnel(markets, s.reject_reason)
    assert passed == 2
    assert reasons["объём 24h < $50,000"] == 2
    assert reasons["не в rewards-программе"] == 1
