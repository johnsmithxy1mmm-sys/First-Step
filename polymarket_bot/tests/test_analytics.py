...
def backdate_trade(ledger, trade_id: int, seconds: float) -> None:
    """Shifts a trade time into the past (simulate an elapsed horizon)."""
...
def test_fills_needing_markout_respects_horizon(ledger):
    ...
    # Fill just happened: the 60s horizon has not arrived yet.
    ...
    # Too early for the 600s horizon.
...
def test_markout_recorded_once_and_stats(ledger):
    ...
    # Duplicate is not written (UNIQUE), the fill left the queue.
    ...
    # Bought at 0.44, a minute later 0.42: markout -0.02 — adverse selection.
...
def test_markout_stats_split_by_strategy(ledger):
    ...
    ledger.record_markout(good, "tok-x", 60, 0.40, 0.43)   # +0.03 in our favor
    ledger.record_markout(bad, "tok-x", 60, 0.50, 0.45)    # -0.05 against
...
def test_compute_report_smoke(ledger):
    """Report builds on a ledger with all data kinds and does not crash on empty."""
...
def test_max_drawdown_math(ledger):
    for equity in (5000, 5500, 4400, 5000):   # peak 5500 -> trough 4400 = -20%
